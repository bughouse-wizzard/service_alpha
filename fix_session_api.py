#!/usr/bin/env python3
"""
Исправление API сессий для работы с SQLite
"""
import os

def create_fixed_session_api():
    """Создает исправленную версию session.py"""
...
"""
    ...
    with open("session_fixed.py", "w") as f:
        f.write(session_py)
    
    print("✅ Создан исправленный файл session_fixed.py")
    ...
    print("4. Создайте резервную копию: cp app/api/session.py app/api/session.py.backup")
    print("5. Замените файл: cp session_fixed.py app/api/session.py")
    print("6. Запустите сервер: source venv/bin/activate && python -m uvicorn app.main:app --host 0.0.0.0 --port 8000")

if __name__ == "__main__":
    create_fixed_session_api()